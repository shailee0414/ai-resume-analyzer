...
export default function Landing() {
  ...
  const primaryHref = isAuthed ? '/analyze' : '/signup';
  const secondaryHref = isAuthed ? '/analyze' : '/login';
  const primaryLabel = isAuthed ? 'Open analyzer' : 'Try free';
  const secondaryLabel = isAuthed ? 'View history' : 'See demo';
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-6 md:py-10">
      <section className="text-center">
        ...
        <h1 className="mt-6 font-serif text-6xl italic leading-tight md:text-6xl">
          Beat the ATS.
          <br />
          Land the interview.
        </h1>
        ...
        <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
          ...
          <Button asChild size="lg" variant="outline">
            <Link to={isAuthed ? historyHref : secondaryHref}>{secondaryLabel}</Link>
          </Button>
        </div>
      </section>
